...
GlobalFonts.registerFromPath(path.join(__dirname, 'src', 'fonts', 'Poppins-Light.ttf'), 'Poppins Light');
GlobalFonts.registerFromPath(path.join(__dirname, 'src', 'fonts', 'Poppins-Medium.ttf'), 'Poppins Medium');
...
client.on( Events.InteractionCreate, async (interaction) =>{
  ...
  async function createChannelTicket(department, roleDepartament) {
    ...
    let responseRowBuilder = new ActionRowBuilder()
      .addComponents(closeResponseButton)

    let staffRole = guild.roles.cache.get("1077610256844193878")
    channelTicket = await guild.channels.create({
  ...
  if(interaction.commandName == 'install-tickets'){
    ...
    .setColor('Random')
    .setAuthor({name: 'IVAO Colombia Tickets', iconURL: client.user.avatarURL()})
    .setURL("https://co.ivao.aero")
    .setDescription(ticketsMessages.descriptionTicket)
    .setTimestamp();

    supportChannel.send({embeds: [ticketCreateEmbed], components: [firstRow]});
  ...
  if(interaction.customId == "selectProblem"){
    ...
    switch (choices) {
      ...
      break;
      default:
        interaction.reply({content: "Interaccion invalida! 📛", ephemeral: true});
        break;
    }
...
client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if(interaction.commandName == "new-training"){
      ...
      try {
        ...
        const background = await Canvas.loadImage('./src/img/EntrenamientosATC.png');

        context.drawImage(background, 0,0, canvas.width, canvas.height);
        context.font = '42px Poppins Medium';
        context.fillStyle = '#ffffff';

        context.fillText(airport, canvas.width / 5 , 650 );
        context.fillText(day, canvas.width / 5 , 800);
        context.fillText(time, canvas.width / 5 , 950 );
        

        const attachment = new AttachmentBuilder(canvas.toBuffer('image/png'),{
          name: 'anuncio-image.png'
        });
        ...
        console.log(error);
        await interaction.reply({
          content: "Se ha producido un error!. Contacta a WM", ephemeral: true
        });
      }
